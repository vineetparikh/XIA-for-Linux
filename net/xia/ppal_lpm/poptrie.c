...
poptrie_init(struct poptrie *poptrie, int sz1, int sz0)
{
    ...
    if ( NULL == poptrie ) {
        /* Allocate new one */
        poptrie = kmalloc(sizeof(struct poptrie), GFP_KERNEL);
        if ( NULL == poptrie ) {
            return NULL;
        }
        (void)memset(poptrie, 0, sizeof(struct poptrie));
        /* Set the flag indicating that this data structure needs kfree() when
           released. */
    ...
    poptrie->nodes = vmalloc(sizeof(poptrie_node_t) * (1 << sz1));
    if ( NULL == poptrie->nodes ) {
        poptrie_release(poptrie);
        return NULL;
    }
    poptrie->leaves = vmalloc(sizeof(poptrie_leaf_t) * (1 << sz0));
    ...
    poptrie->cnodes = kmalloc(sizeof(struct buddy), GFP_KERNEL);
    ...
    poptrie->cleaves = kmalloc(sizeof(struct buddy), GFP_KERNEL);
    ...
    poptrie->dir = kmalloc(sizeof(u32) << POPTRIE_S, GFP_KERNEL);
    if ( NULL == poptrie->dir ) {
        poptrie_release(poptrie);
        return NULL;
    }
    ...
    poptrie->altdir = kmalloc(sizeof(u32) << POPTRIE_S, GFP_KERNEL);
             ...
             struct poptrie_stack *stack, u32 *root, int alt)
{
    ...
    if ( stack->idx < 0 ) {
        cnodes = kmalloc(sizeof(struct poptrie_node), GFP_KERNEL);
        if ( NULL == cnodes ) {
            return -1;
        }
    ...
    cnodes = kmalloc(sizeof(struct poptrie_node), GFP_KERNEL);
#else
    cnodes = kmalloc(sizeof(struct poptrie_node) << (POPTRIE_S - 6), GFP_KERNEL);
#endif
    if ( NULL == cnodes ) {
        return -1;
    }
              ...
              poptrie_node_t *n, poptrie_leaf_t *leaf)
{
    ...
    nodes = kmalloc(sizeof(*nodes) * (1 << 6), GFP_KERNEL);
    if (!nodes)
        return -1;

    children = kmalloc(sizeof(*children) * (1 << 6), GFP_KERNEL);
    if (!children) {
	ret = -1;
        goto nodes;
    }

    leaves = kmalloc(sizeof(*leaves) * (1 << 6), GFP_KERNEL);
    if (!children) {
	ret = -1;
        goto children;
    }
...
{
    if ( NULL == *node ) {
        *node = kmalloc(sizeof(struct radix_node), GFP_KERNEL);
        if ( NULL == *node ) {
            /* Memory error */
            return -1;
        }
